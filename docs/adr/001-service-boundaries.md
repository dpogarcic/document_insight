...
- Configuration explicitly selects a supported implementation for each capability, for example `EMBEDDING_PROVIDER=local_bge`, `RERANKER_PROVIDER=cohere`, or `GENERATION_PROVIDER=mistral`. A base URL, API key, and model name are settings for the selected provider, not a mechanism for inferring its protocol.
