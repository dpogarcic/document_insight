...
- Configuration explicitly selects a supported implementation for each capability, for example `EMBEDDING_PROVIDER=openai_compatible`, `RERANKER_PROVIDER=cohere`, or `GENERATION_PROVIDER=openai_compatible`. A base URL, API key, and model name are settings for the selected provider, not a mechanism for inferring its protocol.
...
- Provider choices are independent. The first supported configuration will use BGE embeddings and a generation model through the `openai_compatible` provider protocol; additional adapters can be added without changing application services. Whether a selected endpoint is local, private-network, or hosted is deployment configuration, not an application provider choice.
