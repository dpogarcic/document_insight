...
- Provider choices are independent. The first supported configuration uses Mistral Embed for embeddings and pinned Mistral-hosted chat models, selected through explicit capability profiles rather than named in code, for structured LLM reranking and generation (the seeded profiles use Ministral 3B). The generation and reranking adapters use the OpenAI Agents SDK's Chat Completions model shape against Mistral's API with tracing disabled; embeddings use a separate typed adapter because the Agents SDK is not an embeddings client.
