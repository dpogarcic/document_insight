...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE evaluation_suite_revisions ADD COLUMN IF NOT EXISTS evaluation_tenant_id uuid"
    )
    op.execute(
        "ALTER TABLE evaluation_suite_revisions ADD COLUMN IF NOT EXISTS corpus_version_ids json"
    )
    op.execute(
        "ALTER TABLE evaluation_suite_revisions ADD COLUMN IF NOT EXISTS dataset_fingerprint varchar(64)"
    )
    ...
    op.execute(
        "UPDATE evaluation_suite_revisions SET evaluation_tenant_id = '00000000-0000-0000-0000-000000000000' WHERE evaluation_tenant_id IS NULL"
    )
    op.execute(
        "UPDATE evaluation_suite_revisions SET corpus_version_ids = '[]'::json WHERE corpus_version_ids IS NULL"
    )
    op.execute(
        "UPDATE evaluation_suite_revisions SET dataset_fingerprint = repeat('0', 64) WHERE dataset_fingerprint IS NULL"
    )
    op.execute(
        "ALTER TABLE evaluation_suite_revisions ALTER COLUMN evaluation_tenant_id SET NOT NULL"
    )
    op.execute(
        "ALTER TABLE evaluation_suite_revisions ALTER COLUMN corpus_version_ids SET NOT NULL"
    )
    op.execute(
        "ALTER TABLE evaluation_suite_revisions ALTER COLUMN dataset_fingerprint SET NOT NULL"
    )

    op.execute(
        "ALTER TABLE evaluation_test_cases ADD COLUMN IF NOT EXISTS relevance_complete boolean"
    )
    op.execute(
        "UPDATE evaluation_test_cases SET relevance_complete = false WHERE relevance_complete IS NULL"
    )
    ...
    op.execute(
        "ALTER TABLE evaluation_case_results ADD COLUMN IF NOT EXISTS lexical_cohort_ranked_ids json"
    )
    op.execute("ALTER TABLE evaluation_case_results ADD COLUMN IF NOT EXISTS measurements json")
    op.execute(
        "ALTER TABLE evaluation_case_results ADD COLUMN IF NOT EXISTS answer_quality_score double precision"
    )
    op.execute(
        "ALTER TABLE evaluation_case_results ADD COLUMN IF NOT EXISTS answer_quality_note varchar(2000)"
    )
    op.execute("UPDATE evaluation_case_results SET variant = 'legacy' WHERE variant IS NULL")
    op.execute(
        "UPDATE evaluation_case_results SET lexical_cohort_ranked_ids = '{}'::json WHERE lexical_cohort_ranked_ids IS NULL"
    )
    op.execute(
        "UPDATE evaluation_case_results SET measurements = '[]'::json WHERE measurements IS NULL"
    )
    op.execute("ALTER TABLE evaluation_case_results ALTER COLUMN variant SET NOT NULL")
    op.execute(
        "ALTER TABLE evaluation_case_results ALTER COLUMN lexical_cohort_ranked_ids SET NOT NULL"
    )
    ...
    op.execute(
        "ALTER TABLE evaluation_gate_reviews ADD COLUMN IF NOT EXISTS threshold_revision varchar(64)"
    )
    op.execute(
        "UPDATE evaluation_gate_reviews SET threshold_revision = 'legacy' WHERE threshold_revision IS NULL"
    )
    op.execute("ALTER TABLE evaluation_gate_reviews ALTER COLUMN threshold_revision SET NOT NULL")

    op.execute(
        "GRANT SELECT, INSERT ON evaluation_suite_revisions, evaluation_test_cases, evaluation_runs, evaluation_case_results, evaluation_aggregates, evaluation_gate_reviews TO di_profile_operator"
    )
    op.execute("GRANT UPDATE (is_active) ON evaluation_suite_revisions TO di_profile_operator")
    op.execute(
        "GRANT UPDATE (status, started_at, completed_at, error_message, enqueued_at, heartbeat_at, comparison_valid) ON evaluation_runs TO di_profile_operator"
    )
    op.execute(
        "GRANT UPDATE (answer_quality_score, answer_quality_note) ON evaluation_case_results TO di_profile_operator"
    )
